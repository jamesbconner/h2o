...
public abstract class Statistic {
  ...
  public abstract void addRow(Data adapter);
...
class Numeric extends Statistic {
  ...
  class ColumnStatistic {
    ...
    void addDataPoint(Data row) { dists[1][row.classOf()] += row.weight(); }    
    ...
    SplitInfo bestSplit() {
      ...
      double last = data.seek(rows_[rowsSize_-1]).getD(column);
      double currSplit =  data.seek(rows_[0]).getD(column);
      ...
      for (int i =0; i< rowsSize_; i++) {
        double s = data.seek(rows_[i]).getD(column);
        if (s > currSplit) {
   ...
   * @param data 
   */
  @Override public void addRow(Data data) {
    for (ColumnStatistic s: columns_)
      s.addDataPoint(data);
    // add the row to the list of rows valid for this node. 
    grow();
    rows_[rowsSize_++] = data.rows();
  }
  private final void grow() { 
    if (rowsSize_==rows_.length) {
      int sz = rowsSize_ * 2;      
      rows_ = Arrays.copyOf(rows_, sz > data.rows()? data.rows() : sz);
    }
  }
  ...
  public Numeric(Data data) {
    rows_ = new int[100];
    columns_ = new ColumnStatistic[data.features()];
    this.data = data;
    pickColumns();
  }
  ...
  private void pickColumns() {
    A: for(int i=0;i<data.features();) {
      columns_[i]=new ColumnStatistic(data.random_.nextInt(data.columns()), data.classes());
  ...
  static final class SplitClassifier implements Classifier {
    ...
    public int classify(Data data) { return data.getD(column)<=value? 0:1; }
    
    public int numClasses()  { return 2; }
    public String toString() { return "col="+column+", val="+value; }
  }
  
 private double get(int i, byte column) { data.seek(i); return data.getD(column); }  
  //OJDK6
