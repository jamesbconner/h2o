...
class EntropyStatistic extends BaseStatistic {
  ...
  @Override protected Split columnSplit(int colIndex) {
    ...
    double weight = aggregateColumn(colIndex, dists_[1]);
    for (double d: dists_[1])
      System.out.print(d+" ");
    if (weight == 0)
      System.exit(0);
    int maxIndex = Utils.maxIndex(dists_[1]);
    if (dists_[1][maxIndex]/weight >= 1-MIN_ERROR_RATE) {
      System.out.println("class "+maxIndex);
    ...
    //System.out.println(colIndex + " - " + split);
    if (split == -1) {
      System.out.println("impossible "+maxIndex);  
      return Split.impossible(maxIndex);
    } else {
      System.out.println();  
      return new Split(colIndex,split,bestFit);
    }
  }
}
