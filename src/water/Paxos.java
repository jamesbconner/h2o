...
public abstract class Paxos {
  ...
  static synchronized void doHeartbeat( H2ONode h2o ) {
    ...
    if( cloud._memset.contains(h2o) ) {
      if( H2O.isIDFromPrevCloud(h2o) ) {
        ...
        HashSet<H2ONode> members = Sets.newHashSet(_state._members);
        print("hart: is member but did not get the news1",cloud._memset);
        print("hart: is member but did not get the news2", members);
        assert cloud._memset.containsAll(members); // We expect only to add nodes
  ...
  static void doChangeAnnouncement( H2O cloud ) {
  ...
  static synchronized int doPromise( State state, H2ONode h2o ) {
    ...
    HashSet<H2ONode> prior_value = Sets.newHashSet(state._members);
  ...
  static synchronized int doAccept( State state, H2ONode h2o ) {
    ...
    if( LEADER != H2O.SELF )
      return print("do  : Nothing; Accept but I am not Leader, no need to send Accepted",PROPOSED_MEMBERS);
  ...
  static synchronized int doAccepted( State state, H2ONode h2o ) {
    ...
    HashSet<H2ONode> members = Sets.newHashSet(state._members);
  ...
  static int print( String msg, H2ONode h2o ) {
    return print(msg, Sets.newHashSet(h2o), "");
  }
