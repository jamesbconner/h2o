 ...
 *
 * If it is a simple value, task is sent to the home of the value and the value is simply stored on hdfs.
 * For arraylets, chunks are stored in order by their home nodes. Each node continues storing chunks 
 * until the next to be stored has different home in which case the task is passed to the home node of that chunk.
 * 
 * Optionally, progress can be monitored by passing a key to a value containing number of chunks stored. If not null, this value
 * will be updated as additional chunks are stored. 
 * 
 * @author tomasnykodym
 *
 */
...
public class TaskStore2HDFS extends RemoteTask {  
  ...
  public static class Serializer extends RemoteTaskSerializer<TaskStore2HDFS> {
    @Override public int  write(TaskStore2HDFS tsk, byte[] buf, int off ) {
    ...
    @Override public TaskStore2HDFS  read(byte[] buf, int off ) { 
      return new TaskStore2HDFS(UDP.get8(buf, off),UDP.get8(buf, off+8),(buf[off+16] == 1)?Key.read(buf, off+17):null); 
    }
    @Override public int wire_len(TaskStore2HDFS tsk) {
      return 16 + 1 + ((tsk._progressK != null)?tsk._progressK.wire_len():0);}
    
    @Override public void write(TaskStore2HDFS task, DataOutputStream dos) throws IOException{
      dos.writeLong(task._indexFrom);
      dos.writeLong(task._indexTo);
      dos.writeBoolean(task._progressK != null);
      if(task._progressK != null)task._progressK.write(dos);
    }    
    @Override public TaskStore2HDFS read(DataInputStream dis) throws IOException {
  ...
  public void compute() {        
    ...
    // store the rest
    if(_indexFrom < _indexTo) new TaskRemExec<TaskStore2HDFS>(_key.home_node(),this,_key);
  ...
  public static class AtomicMax extends Atomic {
    ...
    public static class Serializer extends RemoteTaskSerializer<AtomicMax> {
      ...
      @Override public int  wire_len(AtomicMax a) { return 8; }
      @Override public int  write(AtomicMax a, byte[] buf, int off ) {
        UDP.set8(buf, off, a._myVal);
        return off + 8;
      }
      @Override public void write( AtomicMax a, DataOutputStream dos ) throws IOException {
        dos.writeLong(a._myVal);
      }
      @Override public AtomicMax read( byte[] buf, int off ) {
        return new AtomicMax(UDP.get8(buf,off));      
      }
      @Override public AtomicMax read( DataInputStream dis ) throws IOException {
