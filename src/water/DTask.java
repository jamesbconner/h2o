...
public abstract class DTask<T> extends CountedCompleter implements Freezable {
  ...
  // Make a DTask
  static final DTask make( String clazz, AutoBuffer ab ) {
    // Make a local instance and call map on it
    Exception e=null;
    try {
      DTask dt = (DTask)Class.forName(clazz).newInstance();
      return (DTask)dt.read(ab);
    }
  ...
  public AutoBuffer write(AutoBuffer bb) { throw barf(); }
  public <F extends Freezable> F read(AutoBuffer bb) { throw barf(); }
}
