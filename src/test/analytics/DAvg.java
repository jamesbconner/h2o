 ...
 * 
 * It runs of distributed csv parser and avg computation of data. 
 * Currently avg is computed for all numerical columns.  
 * 
 * To use, simply subclass it and pass correct values to constructor (see PokerAvg.java)
 * 
 * @author tomas
 *
 * @param <T>
...
public abstract class DAvg<T> extends DRemoteTask{
  ...
  public int nvalues(){return _values.length;}
  public double value(int i){return _values[i];}
  
  void processColumn(int i, T r){
    try{  
      if(_isArray){
      ...
      e.printStackTrace();
      throw new Error("unexpected exceptionwhen accessing column " + i + ", " + e);
    }
  }
  
  public DAvg(T csvRecord, String [] columns, CSVParserSetup setup) throws NoSuchFieldException, SecurityException{
  ...
  public void map(Key key) {
    ...
    if(UKV.get(nextKey) == null) nextKey = null;
    try {
      ValueCSVRecords<T> records = new ValueCSVRecords<T>(key,nextKey, index, _csvRecord, _columns, _setup);
      assert _N == 0;
      for(T r:records){
        ++_N;
  ...
  public void reduce(RemoteTask drt) {
    if(drt instanceof DAvg<?>){
      DAvg<?> other = (DAvg<?>)drt;
      assert _values.length == other._values.length;
      double d = 1.0/(double)(_N + other._N);
      for(int i = 0; i < _values.length; ++i){         
        _values[i] = (_values[i]*_N +  other._values[i]*other._N)*d;        
      }
      _N += other._N;
    } else throw new Error("unexpected task type passed in reduce " + drt.getClass().toString());
  ...
  // TODO temporary hack, need remote task to provide a way how to get current key + next key for csv parsing
  // right now - I take it dircetly from the key assuming it is an arraylet chunk and assuming the creation of arraylet keys does not change 
