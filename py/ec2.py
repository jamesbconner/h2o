...
def terminate_instances(reservation):
    if not reservation: return
    conn = boto.connect_ec2()
    conn.terminate_instances([ i.id for i  in reservation.instances])

def hosts_for_reservation(reservation, key_filename=PEM):
    try:
        return [ Ec2Host(i, key_filename=key_filename) for i in reservation.instances ]
