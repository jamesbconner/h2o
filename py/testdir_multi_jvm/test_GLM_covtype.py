...
class Basic(unittest.TestCase):
    ...
    def test_GLM_covtype(self):
        ...
        # L2 
        kwargs = {'y': y, 'x_value': 0, 'case': 1, 'alpha': 0, 'lambda': 0, 'max_iter': max_iter}
        start = time.time()
        ...
        # Elastic
        kwargs = {'y': y, 'x_value': 0, 'case': 1, 'alpha': 0.5, 'lambda': 1e-4, 'max_iter': max_iter}
        start = time.time()
        glm = h2o_cmd.runGLMOnly(parseKey=parseKey, timeoutSecs=120, **kwargs)
        ...
        # L1
        kwargs = {'y': y, 'x_value': 0, 'case': 1, 'alpha': 1.0, 'lambda': 1e-4, 'max_iter': max_iter}
        start = time.time()
        glm = h2o_cmd.runGLMOnly(parseKey=parseKey, timeoutSecs=120, **kwargs)
